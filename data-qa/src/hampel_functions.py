...
def outlier_function_hampel_filter(config, station_id, variable, data, hampel_filter_measurements):
    ...
    # Create a second list without last element (current sample), # Create full list, reason: hampel is not good identifying
    # outliers at the start or end of a list
    data_list_end = data_list[:-1] if len(data_list) >= 2 else data_list
    data_list_end.reverse()
    data_list = data_list + data_list_end
...
def hampel_filter_module(config, station_id, analysis, hampel_filter_measurements, anomaly_status, start_from_0 = False):
    ...
    if start_from_0 == True:
        ...
        # Get historic data
        # historic_data = query_historical_all_data(f'urn:ngsi-ld:AquaSpice:measurementStation:{short_id}', offset=1700, limit=query_points)
        historic_data = aquaspice_utils.query_historical_data_lastN(f'urn:ngsi-ld:AquaSpice:measurementStation:{short_id}', query_points)
        ...
        for variable in analysis["analyzedProperties"]:
            if variable != "location":
                ...
                if historic_data != None:
                    ...
                    # aux_func.logMessage(f"Len dates: {len(historic_data['index'][:2000])}")
                    # aux_func.logMessage(f"Len values: {len(next(e for e in historic_data['attributes'] if e['attrName'] == variable)['values'][:2000])}")
