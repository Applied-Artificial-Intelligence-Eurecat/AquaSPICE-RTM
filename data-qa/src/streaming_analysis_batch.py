...
def process_reading(reading, produce_anomaly, produce_corrected_reading):
    """
    {"id", "type", "subscriptionId", "@context", "notifiedAt", "data" : [{reading}]}
    reading tiene el formato:
    {'id': 'urn:ngsi-ld:AquaSpice:measurementStation:B', 'type': 'MeasurementStation', 
    'conductivity': {'type': 'Property', 'value': -186.955756, 'observedAt': '2022-08-17T07:51:43.000Z'}, 
    'temperature': {'type': 'Property', 'value': 26.644737, 'observedAt': '2022-08-17T07:51:43.000Z'}, 
    'location': {'type': 'geo:json', 'value': {'type': 'Point', 'coordinates': [37.319197, -5.992914]}}, 
    'time_index': '2022-08-17T07:51:43+00:00', 'location_centroid': {'type': 'geo:point', 'value': '-5.992914, 37.319197'}}
    """
    ...
    #for subscriptionId in [x["subscription_id"] for x in aquaspice_utils.config["analysis"]]:
    #for subscriptionId in ["urn:ngsi-ld:Subscription:watercps_detection_1"]:
    #for subscriptionId in ["urn:ngsi-ld:Subscription:hampel_anomaly_detection_1", "urn:ngsi-ld:Subscription:z_score_detection_1"]:
    for subscriptionId in ["urn:ngsi-ld:Subscription:watercps_detection_1"]:
        ...
        # Updates the last time data was received (for each station)
    ...
    #aux_func.logMessage(f"====== Corrected depth value SENT: {property_correction['depth']}")
    '''    
    else:
        aux_func.logMessage("---X Unknown subscription, the incoming package is ignored", "error")
        pass
    '''
...
def manage_sliding_window_dataframe(station_id, property_name, algorithm, value, date, dict_reset = False):
    ...
    #aux_func.logMessage("Entered manage sliding window dataframe.")
    ...
    if dict_reset == False:
        if algorithm == "z_score":
            ...
            #aux_func.logMessage(entities_data['z_score_measurement_' + str(short_id)][property_name]["data"])
...
def _produce_corrected_reading(id, entityType, reading, corrected_variables, analysis, is_outlier, reason_watercps):
    ...
    aux_func.logMessage("Initiated produce_corrected_reading, reason dict:")
    ...
    properties_to_iterate = ["conductivity", "temperature", "depth"]

    # Add corresponding properties to the corrected reading, property_nameRaw and property_nameCorrected
    ...
    # Print body
    #aux_func.logMessage("---> Upsert body:")
    aux_func.logMessage(body)

    aquaspice_utils.upsert_context_broker(body)

@scheduler.task("interval", id = "cadency_check", seconds = 2700, misfire_grace_time = 300)
def cadency_monitoring():
    ...
    for key in last_date_received:
        ...
        if difference > aquaspice_utils.config["data_cadency_anomaly_threshold"]:
            ...
            aux_func.logMessage(f"-X Anomaly of received time DETECTED on {str(key)}: Difference of last received data: {str(difference)}", kind = "warning")
        else:
            #aux_func.logMessage(f"-> Anomaly of received time NOT detected on {str(key)} : Difference of last received data: {str(difference)}", kind = "debug")
            pass
...
if __name__ == "__main__":
    ...
    '''
    #global last_observedAt_received

    aquaspice_utils.init()
    
    # Create subscriptions
    aux_func.logMessage("--> Subscribe to defined topics (json file")
    #last_observedAt_received = {key: {} for key in [x["subscription_id"] for x in aquaspice_utils.config["analysis"]]}
